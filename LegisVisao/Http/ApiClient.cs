...
using NLog;
using LegisVisao.Exceptions;

namespace LegisVisao.Http
{
    public class ApiClient
    {
        ...
        public ApiClient(HttpClient httpClient, ILogger logger = null)
        {
        ...
        public async Task<T> GetAsync<T>(string url, CancellationToken cancellationToken = default)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.Error("Erro ao chamar {Url}: {StatusCode} - {Error}", url, (int)response.StatusCode, errorContent);
            ...
            {
                _logger.Error(ex, "Falha ao desserializar JSON para {Type}. JSON recebido: {Json}", typeof(T).Name, json);
